...
const addWeekToDate = (dateObj: Date, numberOfWeeks: number) => {
...
async function main() {
  ...
  const weekFromNow = addWeekToDate(new Date(), 1);
  const twoWeekFromNow = addWeekToDate(new Date(), 2);
  const monthFromNow = addWeekToDate(new Date(), 4);
  ...
  for (const test of course.tests) {
    await prisma.testResult.create({
      data: {
        ...
        result: testResultsShakuntala[counter] as number,
      },
    });
    ...
    await prisma.testResult.create({
      data: {
        ...
        result: testResultsDavid[counter] as number,
      },
    });
